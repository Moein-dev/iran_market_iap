...
/**
 * Myket billing implementation
 * Handles in-app billing operations for Myket market
 */
class MyketBilling(private val context: Context) {
    companion object {
        private const val TAG = "MyketBilling"
        private const val MYKET_PACKAGE = "ir.mservices.market"
        private const val MYKET_BILLING_SERVICE = "ir.mservices.market.service.InAppBillingService.BIND"
    ...
    /**
     * Service connection for Myket billing service
     */
    ...
    fun connect(): Boolean {
        return try {
            ...
            Log.d(TAG, "Checking if Myket app is installed with package: $MYKET_PACKAGE")
            ...
            val appInstalled = try {
                ...
                val applicationInfo = context.packageManager.getApplicationInfo(MYKET_PACKAGE, 0)
                Log.d(TAG, "Myket app found: ${applicationInfo.packageName}")
                true
            } catch (e: Exception) {
                // If that fails, try to get package info
                try {
                    val packageInfo = context.packageManager.getPackageInfo(MYKET_PACKAGE, 0)
                    Log.d(TAG, "Myket app found via package info: ${packageInfo.packageName}")
                    true
                } catch (e2: Exception) {
                    Log.w(TAG, "Myket app not found: $e2")
                    false
                }
            }
    ...
    fun isBillingSupported(): Boolean {
        return try {
            if (!isConnected) {
                Log.w(TAG, "Myket billing service not connected")
                return false
            }
